...
document.querySelector("#phone-form").addEventListener("keypress", function (evt) {
  if (evt.which != 8 && evt.which != 0 && (evt.which < 48 || evt.which > 57 && evt.which != 43) || this.value.length >= 12)
  {
      evt.preventDefault();
  }
});
...
function sendEmail() {
  ...
  window.open('mailto:' + email + '?subject=' + name + ' -> ' + number + '&body=' + body);
}

//verify when the user click submit do sendmail function
document.getElementById("submit-form").addEventListener("click", function (evt) {
  if (document.getElementById("name-form").value == "" || document.getElementById("email-form").value == "" || document.getElementById("phone-form").value == "" || document.getElementById("myTextArea").value == "") {
    evt.preventDefault();
